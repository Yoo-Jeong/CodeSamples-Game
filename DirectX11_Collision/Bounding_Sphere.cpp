...
_bool CBounding_Sphere::Intersect_Ray(const RAY& In_Ray, float* Out_pDistance)
{
	...
	if (MyMath::Is_SphereToRayCollision(m_pSphere->Center, m_pSphere->Radius, vOrigin, vDirection, fDistance))
	{
		if (Out_pDistance)
		{
...
_vector CBounding_Sphere::Get_CurrentPosition()
{
	_vector vResult = XMVectorSetW(XMLoadFloat3(&m_pSphere->Center), 1.f);
	return vResult;
}
